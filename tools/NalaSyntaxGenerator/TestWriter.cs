...
namespace NalaSyntaxGenerator
{
    internal class TestWriter : AbstractFileWriter
    {
        ...
        private void WriteFile()
        {
            ...
            WriteLine("namespace Nala.Tests");
            OpenBlock();
        ...
        private void WriteNodeGenerator(Node node, bool isGreen)
        {
            ...
            var nalaNamespace = isGreen ? "Nala.Syntax.InternalSyntax." : "";

            var strippedName = StripPost(node.Name, "Syntax");

            WriteLine("private static {0}{1} Generate{2}()", nalaNamespace, node.Name, strippedName);
            OpenBlock();

            //instantiate node
            {
                Write("return {0}SyntaxFactory.{1}(", nalaNamespace, strippedName);
                ...
                foreach (var field in nodeFields)
                {
                    ...
                    else if (field.Type == "SyntaxToken")
                    {
                        ...
                        {
                            Write("{0}SyntaxFactory.Identifier(\"{1}\")", nalaNamespace, field.Name);
                        }
                        else if (kind == "StringLiteralToken")
                        {
                            Write("{0}SyntaxFactory.Literal({1}\"string\", \"string\"{2})", nalaNamespace, leadingTrivia, trailingTrivia);
                        }
                        else if (kind == "CharacterLiteralToken")
                        {
                            Write("{0}SyntaxFactory.Literal({1}\"a\", 'a'{2})", nalaNamespace, leadingTrivia, trailingTrivia);
                        }
                        else if (kind == "NumericLiteralToken")
                        {
                            Write("{0}SyntaxFactory.Literal({1}\"1\", 1{2})", nalaNamespace, leadingTrivia, trailingTrivia);
                        }
                        else
                        {
                            Write("{0}SyntaxFactory.Token(SyntaxKind.{1})", nalaNamespace, ChooseValidKind(field));
                        }
                    }
                    else if (field.Type == "NalaSyntaxNode")
                    {
                        Write("{0}SyntaxFactory.IdentifierName({0}SyntaxFactory.Identifier(\"{1}\"))", nalaNamespace, field.Name);
                    }
                    else
                    {
