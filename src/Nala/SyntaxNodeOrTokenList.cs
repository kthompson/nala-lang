...
namespace Nala
{
    ...
    public struct SyntaxNodeOrTokenList : IEquatable<SyntaxNodeOrTokenList>, IReadOnlyCollection<SyntaxNodeOrToken>
    {
        ...
        internal void CopyTo(int offset, SyntaxNode[] array, int arrayOffset, int count)
        {
        ...
        public SyntaxNodeOrTokenList InsertRange(int index, IEnumerable<SyntaxNodeOrToken> nodesAndTokens)
        {
            ...
            var syntaxNodeOrTokens = nodesAndTokens.ToList();
            ...
            nodes.InsertRange(index, syntaxNodeOrTokens);
            return CreateList(nodes[0].UnderlyingNode, nodes);
        }

        private static SyntaxNodeOrTokenList CreateList(SyntaxNode creator, List<SyntaxNodeOrToken> items)
        {
            if (items.Count == 0)
            {
            ...
            if (newGreen.IsToken)
            {
                newGreen = creator.CreateList(new[] { newGreen });
            }

            return new SyntaxNodeOrTokenList(newGreen);
        }
        ...
        {
            return _node == null
                ? Enumerable.Empty<SyntaxNodeOrToken>().GetEnumerator()
                : this.GetEnumerator();
        }
